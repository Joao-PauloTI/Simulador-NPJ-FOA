					...
					<tr>
						<td>
							<label for="DE_aR1">
								<select
									class="custom-select"
									id="DE_aR1"
									v-model="arAdvogadoRepresentante"
									v-on:change="gerarNome"
								>
						...
						<td>
							<input
								...
								v-model="arNome"
								readonly
								v-autowidth="{maxWidth: '200px', minWidth: '140px', comfortZone: 0}"
								style="border: 0; margin-top: 5px; outline: none"
							/>
						</td>
						<td>
							<button
								...
								v-if="arAdvogadoRepresentante === 'Advogado' && arEstadoOAB != '' && arNumeroOAB != '' && arNome != ''"
								v-on:click="adicionarAdvogado(arEstadoOAB, arNumeroOAB)"
							>Adicionar</button>
							<button
								type="button"
								class="btn btn-primary"
								v-if="arAdvogadoRepresentante === 'Defensor Público' && arMatricula != '' && arNome != ''"
								v-on:click="adicionarDefensor(arMatricula)"
							>Adicionar</button>
						</td>
					</tr>
				</table>
						...
						<tr v-for="advogado, index in arAdvogadoDados">
							<td>
								<input
									...
									value="Advogado"
									readonly
									v-autowidth="{maxWidth: '200px', minWidth: '20px', comfortZone: 0}"
							...
							<td>
								<input
									type="text"
									name="ad_estadoOAB[]"
									v-bind:value="advogado.arEstadoOAB"
									readonly
									v-autowidth="{maxWidth: '200px', minWidth: '20px', comfortZone: 0}"
									style="border: 0; outline: none"
								/>
								<input
									type="text"
									name="ad_numeroOAB[]"
									v-bind:value="advogado.arNumeroOAB"
									readonly
									v-autowidth="{maxWidth: '200px', minWidth: '20px', comfortZone: 0}"
									style="border: 0; outline: none"
								/>
							...
							<td>
								<a href="#" v-on:click="excluirAdvogado(index)">Excluir</a>
							</td>
						</tr>
						<tr v-for="defensor, index in arDefensorDados">
							...
							<td>
								<input
									type="text"
									name="dp_matricula[]"
									v-bind:value="defensor.arMatricula"
									readonly
									v-autowidth="{maxWidth: '200px', minWidth: '20px', comfortZone: 0}"
									style="border: 0; outline: none"
								/>
							...
							<td>
								<a href="#" v-on:click="excluirDefensor(index)">Excluir</a>
							</td>
						</tr>
					</table>
				</div>
...
export default {
	...
	methods: {
		...
		gerarNome: function() {
			if (
				this.arAdvogadoRepresentante === "Advogado" &&
				this.arEstadoOAB === "RJ" &&
				this.arNumeroOAB === "111111"
			) {
				this.arMatricula = "";
				...
				this.arAdvogadoRepresentante === "Advogado" &&
				this.arEstadoOAB === "SP" &&
				this.arNumeroOAB === "333333"
			) {
				this.arMatricula = "";
				this.arNome = "Daniele Souza";
			} else if (
				this.arAdvogadoRepresentante === "Defensor Público" &&
				this.arMatricula === "4444444444"
			) {
				this.arNumeroOAB = "";
				this.arNome = "Adilson Gustavo";
			} else if (
				this.arAdvogadoRepresentante === "Defensor Público" &&
				this.arMatricula === "5555555555"
			) {
				this.arNumeroOAB = "";
				this.arNome = "Rosenclever Lopes";
			} else if (
				this.arAdvogadoRepresentante === "Defensor Público" &&
				this.arMatricula === "6666666666"
			) {
				this.arNumeroOAB = "";
